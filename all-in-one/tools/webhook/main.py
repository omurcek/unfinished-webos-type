...
class webhook:    
    ...
    def main():
        ...
        t.setTitle("discord@webhooktool:login")
        
        while True:
            while True:
                try:
                    url = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@webhook:login → webhook~ "))
                    ...
                    print("")
                    t.print.warn("Exiting the Lapidary Webhook.")
                    sleep(0.6)
                    import lapidary
                    return lapidary.core.main()
                except Exception:
            ...
            while True:
                try:
                    t.setTitle("discord@webhooktool:menu")
                    choice = input(Colorate.Horizontal(Colors.red_to_purple, 'lapidary@webhook:menu → select~ ', 1))
                    
                    if (choice == "1" or choice == "01"):
                        t.setTitle("discord@webhooktool:sendmessage")
                        webhook.sendMessage(url)
                    elif (choice == "2" or choice == "02"):
                        t.setTitle("discord@webhooktool:spammessage")
                        webhook.spamMessage(url)
                    elif (choice == "3" or choice == "03"):
                        t.setTitle("discord@webhooktool:sendmessage")
                        webhook.rename(url)
                    elif (choice == "4" or choice == "04"):
                        t.setTitle("discord@webhooktool:webhookinfo")
                        t.clear()
                        t.print.logo("Webhook")
                        webhook.menu()
    ...
    def sendMessage(url):
        message = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@webhook:sendmessage → message~ ", 1))
    ...
    def spamMessage(url):
        message = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@spammessage → message~ ", 1))
        ratelimit = int(input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@rename → timeout~ ")))
        ...
        while True:                            
            try:
                ...
                print("")
                t.print.warn("Exiting the Lapidary webhook spammer...")
                sleep(0.7)
                return webhook.main()
            
    def rename(url):
        name = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@webhook:rename → name~ "))
    ...
    def changePFP(url):
        ...
        if image_path is None or image_path == "":
            image_path = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@changepfp → path/url~ "))
