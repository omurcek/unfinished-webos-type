...
class nuker:
    ...
    async def nuke_server(self, guild):
        try:
            ...
            create_tasks = [self.create_and_populate_channel(guild, random.choice(channel_names)) for _ in range(20)]
    ...
    @staticmethod
    def main():
        token = pwinput.pwinput(Colorate.Horizontal(Colors.red_to_purple, "lapidary@nuker → token~ ", 1))
        ...
        try:
            if token:
                ...
                t.print.fail("No token provided, exiting Lapidary nuker.")
        except:
            t.print.fail("Invalid token provided, exiting Lapidary nuker.")