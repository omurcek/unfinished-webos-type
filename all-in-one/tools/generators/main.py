...
class generator():
    class nitro():
        ...
        def update_title():
            t.setTitle(f"discord@generator - Unvalid Codes: {generator.nitro.total_requests - generator.nitro.successful_requests} | Valid Codes: {generator.nitro.successful_requests}")
...
{Fore.MAGENTA}01 »{Fore.RESET} Nitro Generator
    ...
    def main():
        ...
        t.setTitle("discord@generator:menu")
        generator.menu()
        # t.print.info("You can exit generator tool by typing 'exit'.")
        
        while True:
            try:
                choice = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@generators:menu → select~ ", 1))
                
                if (choice == "1" or choice == "01"):
                    t.setTitle("discord@generator:nitro")
                    t.clear()
                    t.print.logo("Generator")
                    t.print.info("You can exit nitro generator tool by pressing CTRL + C.")
                    
                    webhook = input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@generator:nitro → webhook~ ", 1))
                    timeout = int(input(Colorate.Horizontal(Colors.red_to_purple, "lapidary@generator:nitro → timeout~ ", 1)))
                    ...
                    while True:
                        try:
                            ...
                            t.print.warn("Exiting the Lapidary nitro generator...")
                            sleep(0.8)
                            generator.main()
                            break
                elif (choice == "exit" or choice == "x"):
                    import lapidary
                    t.print.warn("Exiting the Lapidary generators...")
                    sleep(0.7)
                    lapidary.core.main()
            except KeyboardInterrupt:
                import lapidary
